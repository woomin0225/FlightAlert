...
def cmd_add():
    ...
    print(
        f"알림 추가 완료: {origin} -> {destination} / "
        f"{departure_date} / 목표가 {target_price:,.0f}원"
    )
...
def cmd_list():
    ...
    for alert in alerts:
        ...
        print(
            f"{alert['id']:<4} {alert['email']:<28} {route:<14} "
            f"{alert['departure_date']:<12} {alert['target_price']:>12,.0f} {status}"
        )
    print()
